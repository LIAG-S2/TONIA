...
def plot_data_subplots(data_file, spacing_labels, farmName, refPoints, data_type, cmap='Spectral_r', vmin=10, vmax=1000): 
    ...
    try:
        ...
        print(f"Failed to read data with read_data_Aus_lineIndexed: {e}. Trying read_data_Aus.")
    ...
    # Create a multi-page PDF file to save the plots
    pdf_filename = f"Data_{farmName}_subplot_{data_type}.pdf"
    pdf_pages = PdfPages(pdf_filename)
...
def main():
    ...
    data_type_LOF = 'harmfit & LOF'
    harmfit_onLOF = harmfit_fn(LOF_data_file, spacing_labels, farmName_Original, refPoints, data_type_LOF)
    harmfit_onLOF_file = f"harmfit_on_{data_type_LOF} _{farmName_Original}.txt"
    data_harmfit_LOF = plot_data_subplots(harmfit_onLOF_file, spacing_labels, farmName_Original, refPoints, data_type_LOF)

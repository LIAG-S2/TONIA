...
farmName_Processed = 'Beerfelde Fettke_HLOF'
# farmName_Original = 'Großbeeren_1'
data_type_HLOF = 'LOF'
...
#%%
farmName_Processed = 'Geophilus_1_HLOF'

inv_result_data = np.loadtxt(f'{farmName_Processed}_1Dinv_response_all_lines_combined.txt', delimiter='\t', skiprows=1)

A = np.genfromtxt(inv_result_data, names=True)
fig, ax = plt.subplots(nrows=3, ncols=5, sharex=True, sharey=True, figsize=(10, 5))
norm = LogNorm(10, 1000)
for i, a in enumerate(ax.flat):
    rho = A["rho{:02d}".format(i+1)]
    im = a.scatter(A["x"]-460000, A["y"]-5.8e6, c=rho, s=1, norm=norm, cmap="Spectral_r", alpha=1)
    a.set_aspect(1.0)

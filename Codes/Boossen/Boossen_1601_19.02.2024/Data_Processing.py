...
def read_data_Boossen(filepath):
        EdecDeg, NdecDeg, H, Rho1, Rho2, Rho3, Rho4, Rho5, Rho6, Gamma = np.genfromtxt(filepath, skip_header=1, delimiter=';', unpack=True)
        # EdecDeg, NdecDeg, H, Rho1, Rho2, Rho3, Rho4, Rho6, Gamma = np.genfromtxt(filepath, skip_header=1, delimiter=',', unpack=True)
        Eutm, Nutm, zone, letter = utm.from_latlon(NdecDeg, EdecDeg, 33, 'U')
        # data = np.column_stack((Eutm, Nutm, H, Rho1, Rho2, Rho3, Rho4, Rho5, Rho6, Gamma))
...
def read_data_lineIndexed(filepath):
    EdecDeg, NdecDeg, H, Rho1, Rho2, Rho3, Rho4, Rho5, Rho6, Gamma, line_index = np.genfromtxt(filepath, skip_header=0, delimiter=',', unpack=True)
    # EdecDeg, NdecDeg, H, Rho1, Rho2, Rho3, Rho4, nan_data, Rho5, Gamma, BFI, lfd, Datum, Zeit = np.genfromtxt(filepath, skip_header=1, delimiter=',', unpack=True)
    data = np.column_stack((EdecDeg, NdecDeg, H, Rho1, Rho2, Rho3, Rho4, Rho5, Rho6, Gamma, line_index))
    return data
...
def plot_data(data_file, spacing_labels, farmName, refPoints, data_type, cmap='Spectral_r', vmin=10, vmax=1000):
    # Read original data
    data = read_data_Boossen(data_file)
    # Read processed data
...
def harmfit_fn(data_file, spacing_labels, farmName_Original, refPoints_df, data_type, cmap='Spectral_r', vmin=10, vmax=1000):
    ...
    np.savetxt(txt_filename, harmfitted_array_float64, delimiter=',', header='x,y,h,harmfitted_1,harmfitted_2,harmfitted_3,harmfitted_4,harmfitted_5, harmfitted_6,gamma,line_index', fmt='%0.8f', comments='')
...
def main():
    ...
    filepath_Original = 'LPP_1601_Geophilus_221019_roh_EPSG04326.csv'
    farmName_Original = 'Boossen_1601'
    
    # filepath_Original = 'LPP_1211_Geophilus_170411_roh_EPSG04326.csv'
    # farmName_Original = 'Boossen_1211'
    ...
    kmlFile = 'LPP_1601_Referenzpunkte_gemessen_2023-03-20.kml'
    ...
    spacing_labels_Boossen = ['Spacing 1', 'Spacing 2', 'Spacing 3', 'Spacing 4', 'Spacing 5', 'Spacing 6']
    spacing_labels = ['Spacing 1', 'Spacing 2', 'Spacing 3', 'Spacing 4', 'Spacing 5']

    # Read original data
    # data_original = read_data_Aus(filepath_Original)
    data_original = read_data_Boossen(filepath_Original)
    # downsampled_data = data_original[::2]
    ...
    data_org = plot_data(filepath_Original, spacing_labels_Boossen, farmName_Original, refPoints, data_type_org)
    data_org = plot_data_subplots(filepath_Original, spacing_labels_Boossen, farmName_Original, refPoints, data_type_org)

    # Perform harmfit on original and draw areal plots
    data_lineIndexed_file = f"data_lineIndexed_{farmName_Original}.txt"
    harmfit_onOrg = harmfit_fn(data_lineIndexed_file, spacing_labels_Boossen, farmName_Original, refPoints, data_type_org)
    ...
    zscore_data = zscore_outliers(data_lineIndexed_file, farmName_Original, spacing_labels_Boossen, refPoints)
    
    # Perform harmfit on zscore result and draw areal plots
    zscore_data_file = f" zscore_{farmName_Original}.txt"
    data_type_zScore = 'zscore'
    harmfit_onZscore = harmfit_fn(zscore_data_file, spacing_labels_Boossen, farmName_Original, refPoints, data_type_zScore)
    
    # LOF on Original
    data_lineIndexed_file = f"data_lineIndexed_{farmName_Original}.txt"
    LOF = LOF_outliers(data_lineIndexed_file, spacing_labels_Boossen, refPoints, farmName_Original)
    
    # harmfit on LOF
    LOF_data_file = f"LOF_{farmName_Original}.txt"
    data_type_LOF = 'LOF'
    harmfit_onLOF = harmfit_fn(LOF_data_file, spacing_labels_Boossen, farmName_Original, refPoints, data_type_LOF)

    # subplot plot HLOF
    harmfit_onLOF_file = f"harmfitted_{data_type_LOF}_{farmName_Original}.txt"
    data_harmfit_LOF = plot_procesed_subplots(harmfit_onLOF_file, spacing_labels_Boossen, farmName_Original, refPoints, data_type_LOF)
...
filepath_Original = 'LPP_1601_Geophilus_221019_roh_EPSG04326.csv'
farmName_Original = 'Boossen_1601'
kml_file = 'LPP_1601_Referenzpunkte_gemessen_2023-03-20.kml'

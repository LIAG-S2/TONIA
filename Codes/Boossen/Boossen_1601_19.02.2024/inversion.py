...
def inversion_1D_all_lines(filepath, amVec, farmName):
    ...
    """
    spacing_labels = ['S1', 'S2', 'S3', 'S4', 'S5', 'S6']
...
def read_data_lineIndexed_Boos(filepath):
    EdecDeg, NdecDeg, H, Rho1, Rho2, Rho3, Rho4, Rho5, Rho6, Gamma, line_index = np.genfromtxt(filepath, skip_header=1, delimiter=',', unpack=True)
...
farmName_Processed = 'Boossen_1601_HLOF'
# farmName_Original = 'Großbeeren_1'
data_type_HLOF = 'LOF'
...
def plot_1d_inversion_results_allLines(filepath, amVec, farmName, data_type):
    data = read_data_lineIndexed_Boos(filepath)
    data_no_nan = data[~np.isnan(data).any(axis=1)]
    data_filtered = data_no_nan[data_no_nan[:, -1] != 0] # remove individual points
    selected_lines = np.unique(data_filtered[:,-1])
...
for i, a in enumerate(ax.flat):
    ...
    im = a.scatter(inv_res[:, 0]-x_offset, inv_res[:, 1]-y_offset, c=rho, s=1, norm=norm, cmap="Spectral_r", alpha=1)
    a.set_aspect(1.0)
...
plt.savefig(f'{farmName_Processed}_inversion_results.pdf')
# Save the plot as JPEG
plt.savefig(f'{farmName_Processed}_inversion_results.jpg', dpi=300)
...
with PdfPages(pdf_filename) as pdf:
    for i in range(15):  # Loop through all subplots
        ...
        norm = LogNorm(10, 1000)
        rho = inv_res[:, i + 2]  # Assuming the rho values start from the third column
        im = ax.scatter(inv_res[:, 0] - x_offset, inv_res[:, 1] - y_offset, c=rho, s=0.9, norm=norm, cmap="Spectral_r", alpha=1)
        #ax.set_aspect(1.0)
...
with PdfPages(pdf_filename) as pdf:
    ...
    im = ax.scatter(inv_res[:, 0] - x_offset, inv_res[:, 1] - y_offset, c=chi2, s=0.8, norm=norm, cmap="Spectral_r", alpha=1)
    ax.set_aspect(1.0)

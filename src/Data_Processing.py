...
class DataProcessing:
    ...
    def read_data(self, filepath_Original):
        ########## Read the first few lines to determine the structure of the file (; or , is used?)
        with open(filepath_Original, 'r', encoding='utf-8') as csvfile:
            ...
            data = np.genfromtxt(filepath_Original, names=True, delimiter=delimiter, dtype=None, encoding='utf-8')
            ...
            rho_indices = [i for i, col in enumerate(filtered_data.dtype.names) if col.startswith('Rho') and all(char.isdigit() or char.isspace() or char == '.' or char == '_' for char in col[3:])]
    
            # Extract columns from filtered data based on rho_indices
            rhos = [filtered_data[filtered_data.dtype.names[idx]] for idx in rho_indices]
            ...
            gamma_index = [i for i, col in enumerate(filtered_data.dtype.names) if col == 'Gamma']
            # Extract Gamma column from filtered data
            Gamma = filtered_data[filtered_data.dtype.names[gamma_index[0]]]
            
            ########## Extracting the columns based on specific characters in their headers (only those letters)
            EdecDeg = filtered_data[filtered_data.dtype.names[next(i for i, col in enumerate(filtered_data.dtype.names) if col == 'E' or col == 'x' or col == 'X')]]
            NdecDeg = filtered_data[filtered_data.dtype.names[next(i for i, col in enumerate(filtered_data.dtype.names) if col == 'Y' or col == 'Y' or col == 'N')]]
            H = filtered_data[filtered_data.dtype.names[next(i for i, col in enumerate(filtered_data.dtype.names) if col == 'H')]]
...
# # def main():
# data_processor = DataProcessing()  # Create an instance of DataProcessing
...
# data_org = data_processor.plot_data(filepath_Original, spacing_labels, farmName_Original, refPoints, data_type_org)
...
# data_processor.reference_points_from_kml(kml_file, farmName_Original)

#%% compare data for 2 lines 11 and 14
if 0:
    from matplotlib.backends.backend_pdf import PdfPages
    ...
    def calculate_distances(x, y):
        ...
        # Cumulative sum to get total distance
    ...
    # Read the data file
    data = read_data_lineIndexed(data_lineIndexed_file)
    
    # Find the indices of rows corresponding to lines 11 and 14
    line_11_indices = np.where(data[:, -1] == 11)[0]
    line_14_indices = np.where(data[:, -1] == 14)[0]
    
    # Extract data for lines 11 and 14
    data_line_11 = data[line_11_indices] 
    data_line_14 = data[line_14_indices]
    
    # Plotting and saving to PDF
    with PdfPages(f'compare_2_identical_lines_{farmName_Original}.pdf') as pdf:
        
        # Plotting the location of lines 11 and 14
        plt.figure(figsize=(8, 6))
        
        # Plot for Line 11
        plt.scatter(data_line_11[:, 0], data_line_11[:, 1], c='b', label='Line 11', marker='o', s=5)
        plt.scatter(data_line_14[:, 0], data_line_14[:, 1], c='r', label='Line 14', marker='x', s=10)
        
        plt.title(f'Locations of X and Y for Lines 11 and 14 {farmName_Original}')
        plt.xlabel('X')
        plt.ylabel('Y')
        plt.legend()
        plt.grid(True)
        ...
        # Plotting to compare 2 line data
        data_line_14 = data_line_14[:-1]
    
        
        num_cols = 5  # Number of columns to plot
        fig, axs = plt.subplots(1, num_cols, figsize=(25, 6), sharey=True)
        
        for i in range(num_cols):
            # Filter values greater than 2500 for line 11
            filtered_data_line_11 = data_line_11[data_line_11[:, i+3] <= 2500]
            # Plot filtered data for line 11
            axs[i].plot(filtered_data_line_11[:, 0], filtered_data_line_11[:, i+3], c='b', label=f'Line 11')
        
            # Filter data for line 14 by excluding the largest value
            filtered_data_line_14 = np.delete(data_line_14, max_index_line_14, axis=0)
            # Filter values greater than 2500 for line 14
            filtered_data_line_14 = data_line_14[data_line_14[:, i+3] <= 2500]
            # Plot filtered data for line 14
            axs[i].plot(filtered_data_line_14[:, 0], filtered_data_line_14[:, i+3], c='r', label=f'Line 14')
            
            axs[i].set_title(f'Rho {i+1}')
            axs[i].set_xlabel('Distance between points')
        ...
        plt.tight_layout()
        pdf.savefig(fig)
        plt.savefig(f'compare_2_identical_lines_{farmName_Original}.jpg', dpi=300)
